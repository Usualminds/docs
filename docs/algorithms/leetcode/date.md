...
function nthMagicalNumber(n: number, a: number, b: number): number {
    let mod = 10 ** 9 + 7, left = Math.min(a, b), right = n * Math.min(a, b), lcmNum = lcm(a, b)
...
const dfs = (cache: Array<Array<number>>, a: number, b: number): number => {
    ...
    if (cache[a][b] === 0) {
        cache[a][b] = .25 * (dfs(cache, a - 4, b) + dfs(cache, a - 3, b - 1) + dfs(cache, a - 2, b - 2) + dfs(cache, a - 1, b - 3))
    }
...
- 示例 1:
    - 输入: `order = "cba", s = "abcd"`
    - 输出: `"cbad"`
    - 解释: “a”、“b”、“c”是按顺序出现的，所以“a”、“b”、“c”的顺序应该是“c”、“b”、“a”。
